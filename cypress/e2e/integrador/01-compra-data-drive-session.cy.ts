...
describe("Integrador - Flujo de compra data-driven con sesión", () => {
  ...
  beforeEach(() => {
    ...
    // 1. Ejecuta el helper de autenticación
    loginToSauceDemo();

    // 2. FUERZA la navegación a la página de inventario si usas sesiones
  ...
  compraData.forEach((testCase) => {
    it(`Test 2 - ${testCase.name}`, () => {
      ...
      allure.logStep(
        `Step 1: Ordenar productos según el "orden" indicado: ${testCase.sort}`,
      );
      cartPage.sortProducts(testCase.sort);

      // Step 2: Agregar productos al carrito
      allure.logStep(
        `Step 2: Agregar al carrito según los índices definidos en "pick"`,
      );
      ...
      allure.logStep(
        `Step 3: Validar numero de productos en carrito de acuerdo al parámetro "expectedBadge": ${testCase.expectedBadge}`,
      );
